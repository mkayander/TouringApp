...
export const Sidebar: React.FC<SidebarProps> = ({ routeHook, modalHook }) => {
  const { routeId, setRouteId, activeRoute, routesList, deleteRoute } =
    routeHook;

  return (
    <ProSidebar collapsed={false} breakPoint={"md"}>
      <SidebarHeader>
        <div
          style={{
            padding: "24px",
      ...
      <SidebarContent>
        <Menu iconShape="square">
          ...
          <SubMenu title="Маршруты" icon={<FaRoute />} defaultOpen={true}>
              ...
              >
                ...
                <FaTrash
                  ...
                  onClick={() => {
                    if (
                      window.confirm(
                        `Вы действительно хотите удалить тур "${route.title}"?`
                      )
                    ) {
            ...
            <AddButton
              onClick={() => modalHook.setActiveModal(ModalType.CreateRoute)}
            >
              <FaPlus />
            </AddButton>
          </SubMenu>
